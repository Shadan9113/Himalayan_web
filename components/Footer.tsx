      ...
      <div className="padding-container max-container flex w-full flex-col gap-14">
        <div className="flex flex-col items-start justify-center gap-[10%] md:flex-row">
          <Link href="/" className="mb-10">
            <Image src="/himalaya.png" alt="himalaya" width={74} height={29} />
          </Link>

          <div className="flex flex-wrap gap-10 sm:justify-between md:flex-1">
            {FOOTER_LINKS.map((columns, index) => (
              <FooterColumn title={columns.title} key={`footer-link-column-${index}`}>
                <ul className="regular-14 flex flex-col gap-4 text-gray-30">
                  {columns.links.map((link, linkIndex) => (
                    <Link href="/" key={`footer-link-${index}-${linkIndex}`}>
                      {link}
                    </Link>
                  ))}
                </ul>
              ...
              <FooterColumn title={SOCIALS.title} key="footer-socials">
                <ul className="regular-14 flex gap-4 text-gray-30">
                  {SOCIALS.links.map((link, socialIndex) => (
                    <Link href="/" key={`social-link-${socialIndex}`}>
        ...
        <p className="regular-14 w-full text-center text-gray-30">
          2023 Himalaya Heaven | All rights reserved
        </p>
      </div>
    </footer>
  )
